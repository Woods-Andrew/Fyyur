...
# Imports
# ----------------------------------------------------------------------------#
# The imports I need for my project
import json
...
from classes import db, Venue, Artist, Show
...
#  Venues
#  ----------------------------------------------------------------
# mock data removed, now extracting data from database directly.
@app.route("/venues")
def venues():
    every_venue = (
        Venue.query.with_entities(func.count(Venue.id), Venue.city, Venue.state).group_by(Venue.city, Venue.state).all()
    )

    data = []
    venue_data = []

    for areas in every_venue:
        venue_places = (
...
# Where you serch for any venues
# Allows you to search M but then Music comes up.
...
# allows you to see the page once given "venue_id"
...
# This is the data foe the venues needed to be filled out.
    data = {
        "id": venue.id,
        "name": venue.name,
        "genres": venue.genres,
...
#  Create Venue
#  ----------------------------------------------------------------
# Calling back after finished doing task
@app.route("/venues/create", methods=["GET"])
def create_venue_form():
    form = VenueForm()
    return render_template("forms/new_venue.html", form=form)

# Creating the venue using method post
...
# Deleting the venue once given the id using the Delete method
...
#Searching for artist should bring Atom, when you search A.
